function [p,F,pt,Ft,pt1,Ft1,pt2,Ft2,pt3,Ft3,pt4,Ft4,mdl1,mdl2,pvalv,Fvalv,pvalvr,Fvalvr,Rvall,Rvalr]=evaluate_activation_alt(S,channel_ind,val_freq,lims,timev,samp_size)
close all;
close all hidden;
% %% all sample with exo-skeleton
% S{1}=load([pwd '/Data_res_val_def_bouts_def/Subj 01_Run 3_res_vals.mat']);
% S{2}=load([pwd '/Data_res_val_def_bouts_def/Subj 02_Run 3_res_vals.mat']);
% S{3}=load([pwd '/Data_res_val_def_bouts_def/Subj 03_Run 3_res_vals.mat']);
% S{4}=load([pwd '/Data_res_val_def_bouts_def/Subj 04_Run 2_res_vals.mat']);
% S{5}=load([pwd '/Data_res_val_def_bouts_def/Subj 05_Run 1_res_vals.mat']);
% S{6}=load([pwd '/Data_res_val_def_bouts_def/Subj 06_Run 1_res_vals.mat']);
% S{1}=load([pwd '/probe_data_again_def/Subj 01_Run 3_res_vals.mat']);
% S{2}=load([pwd '/probe_data_again_def/Subj 02_Run 3_res_vals.mat']);
% S{3}=load([pwd '/probe_data_again_def/Subj 03_Run 3_res_vals.mat']);
% S{4}=load([pwd '/probe_data_again_def/Subj 04_Run 2_res_vals.mat']);
% S{5}=load([pwd '/probe_data_again_def/Subj 05_Run 1_res_vals.mat']);
% S{6}=load([pwd '/probe_data_again_def/Subj 06_Run 1_res_vals.mat']);
% S{1}=load([pwd '/probe_data_again_clip_def/Subj 01_Run 3_res_vals.mat']);
% S{2}=load([pwd '/probe_data_again_clip_def/Subj 02_Run 3_res_vals.mat']);
% S{3}=load([pwd '/probe_data_again_clip_def/Subj 03_Run 3_res_vals.mat']);
% S{4}=load([pwd '/probe_data_again_clip_def/Subj 04_Run 2_res_vals.mat']);
% S{5}=load([pwd '/probe_data_again_clip_def/Subj 05_Run 1_res_vals.mat']);
% S{6}=load([pwd '/probe_data_again_clip_def/Subj 06_Run 1_res_vals.mat']);
% S{1}=load([pwd '/probe_data_again_amp_def/Subj 01_Run 3_res_vals.mat']);
% S{2}=load([pwd '/probe_data_again_amp_def/Subj 02_Run 3_res_vals.mat']);
% S{3}=load([pwd '/probe_data_again_amp_def/Subj 03_Run 3_res_vals.mat']);
% S{4}=load([pwd '/probe_data_again_amp_def/Subj 04_Run 2_res_vals.mat']);
% S{5}=load([pwd '/probe_data_again_amp_def/Subj 05_Run 1_res_vals.mat']);
% S{6}=load([pwd '/probe_data_again_amp_def/Subj 06_Run 1_res_vals.mat']);
% S{1}=load([pwd '/probe_data_again_zero/Subj 01_Run 3_res_vals.mat']);
% S{2}=load([pwd '/probe_data_again_zero/Subj 02_Run 3_res_vals.mat']);
% S{3}=load([pwd '/probe_data_again_zero/Subj 03_Run 3_res_vals.mat']);
% S{4}=load([pwd '/probe_data_again_zero/Subj 04_Run 2_res_vals.mat']);
% S{5}=load([pwd '/probe_data_again_zero/Subj 05_Run 1_res_vals.mat']);
% S{6}=load([pwd '/probe_data_again_zero/Subj 06_Run 1_res_vals.mat']);
...
%% left
figure;
imagesc(linspace(0,1,samp_size(2)),S{1}.freql{1},imgaussfilt(squeeze(mean(dataHEALtot,1)),1)/12+0.2)%(2*mat2gray(imgaussfilt(squeeze(mean(dataHEALtot,1)),1))-1)./2);
colormap(jet);
caxis(lims)
set(gca,'FontSize',26);
ylabel('Frequency [Hz]');
...
dataHEALLst=zeros([3,samp_size(1),samp_size(2)]);
dataHEALRst=zeros([3,samp_size(1),samp_size(2)]);
dataHEALLchst=zeros([3,32,samp_size(1),samp_size(2)]);
dataHEALRchst=zeros([3,32,samp_size(1),samp_size(2)]);
dataHEALtotst=zeros([3,samp_size(1),samp_size(2)]);
dataHEALtotchst=zeros([3,32,samp_size(1),samp_size(2)]);
c=1;
data_val_l_st=[];
data_val_r_st=[];
for i=[3]
...
figure;
imagesc(linspace(0,1,samp_size(2)),S{1}.freql{1},imgaussfilt(squeeze(mean(dataHEALtotst,1)),1)/12+0.1)%(2*mat2gray(imgaussfilt(squeeze(mean(dataHEALtotst,1)),1))-1)./2);
colormap(jet);
caxis(lims)
set(gca,'FontSize',26);
ylabel('Frequency [Hz]');
...
figure;
imagesc(linspace(0,1,samp_size(2)),S{1}.freql{1},(2*mat2gray(imgaussfilt(squeeze(mean(squeeze(mean(dataHEALtotchst,1)),1)),1))-1)./2);
colormap(jet);
caxis(lims)
set(gca,'FontSize',26);
ylabel('Frequency [Hz]');
...
colorbar
subplot(122)
topoplot((squeeze(mean(squeeze(mean(squeeze(mean(dataHEALtotchst(:,:,posbeta1:posbeta2,post1:post2),1)),2)),2)))./12+0.1,locs,'electrodes','on','electrodes','labels');
caxis([lims])
colorbar
...
figure;
[p,F]=anova1([valsh',valst'],[zeros([1 3]) ones([1 3])])
A=1;
