...
while ind==0
     EEGL=eeg_emptyset();
     EEGR=eeg_emptyset();
     [accel{ccount},EEG{ccount},ind,indbout,EMG_sal{ccount}]=read_subjects_EEG_Mater_alt_OG(subj,runind,ckcount,1,0,1,sel_emg,2.2,EMG.syncData); %% processing the window on 3 seconds around the Bout detection
     %ind
     if ind==1
         %save([subj '_' runind '_res_vals.mat'],'erspl','itcpl','freql','erspr','itcpr','freqr','tl','tr');
         %% cutting the possible bouts
         break;
     end;
     ...
     if indbout==0
         ...
         [EEGL_set{ccount}.icaweights,EEGL_set{ccount}.icasphere]=runica(EEGL_set{ccount}.data(:,:),'sphering','on','lrate',1e-5,'maxsteps',50,'reset_randomseed','off'); 
         [EEGR_set{ccount}.icaweights,EEGR_set{ccount}.icasphere]=runica(EEGR_set{ccount}.data(:,:),'sphering','on','lrate',1e-5,'maxsteps',50,'reset_randomseed','off'); 
         ...
         EEGL_set{ccount}.trials=100; %% 25 repetitions
         ...
         EEGR_set{ccount}.trials=100;%% 25 repetitions
