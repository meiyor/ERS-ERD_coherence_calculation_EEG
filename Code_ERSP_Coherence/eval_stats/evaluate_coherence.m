function [p,F,data_sal]=evaluate_coherence(S,channel_ind,val_freq,timev,samp_size_2d,sel,lims,sel_limb,plot_sel)
close all;
close all hidden;
...
dataHEALLst=zeros([3,80,samp_size_2d(2)]);
dataHEALRst=zeros([3,80,samp_size_2d(2)]);
dataHEALLchst=zeros([3,32,80,samp_size_2d(2)]);
dataHEALRchst=zeros([3,32,80,samp_size_2d(2)]);
dataHEALtotst=zeros([3,80,samp_size_2d(2)]);
dataHEALtotchst=zeros([3,32,80,samp_size_2d(2)]);
c=1;
...
if plot_sel==1
      ...
      [p,F]=anova1([valsh',valst'],[zeros([1 3]) ones([1 3])],'off')
end;
%%A=1;