...
%% plotting here
figure;
imagesc(linspace(0,1,samp_size(2)),S{1}.freql{1},(2*mat2gray(imgaussfilt(squeeze(mean(dataHEALtot,1)),1.5))-1)./2);
colormap(jet);
caxis(lims)
set(gca,'FontSize',20);
ylabel('Frequency [Hz]');
...
figure;
imagesc(linspace(0,1,samp_size(2)),S{1}.freql{1},(2*mat2gray(imgaussfilt(squeeze(mean(dataHEALtotst,1)),1.5))-1)./2);
colormap(jet);
caxis(lims)
set(gca,'FontSize',20);
ylabel('Frequency [Hz]');
